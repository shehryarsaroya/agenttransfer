...
func (s *Store) TouchConnectInstance(name string) error {
	_, err := s.DB.Exec(`UPDATE connect_instances SET last_seen=? WHERE name=?`, now(), name)
	return err
}
...
func (s *Store) ReapConnectInstances(graceNew, graceIdle time.Duration) ([]string, error) {
	n := now()
	rows, err := s.DB.Query(`SELECT name FROM connect_instances
		WHERE (last_seen=0 AND created_at<=?) OR (last_seen>0 AND last_seen<=?)`,
		n-int64(graceNew.Seconds()), n-int64(graceIdle.Seconds()))
	if err != nil {
		return nil, err
	}
	var names []string
	...
	for _, name := range names {
		if _, err := s.DB.Exec(`DELETE FROM connect_mail WHERE name=?`, name); err != nil {
			return names, err
		}
		if _, err := s.DB.Exec(`DELETE FROM connect_instances WHERE name=?`, name); err != nil {
			return names, err
		}
	}
	return names, nil
...
func (s *Store) EnqueueConnectMail(name string, rcpts []string, raw []byte, maxMsgs, maxBytes int64) error {
	...
	if err := s.DB.QueryRow(`SELECT COUNT(*), SUM(LENGTH(raw)) FROM connect_mail WHERE name=?`, name).Scan(&msgs, &bytes); err != nil {
		return err
	}
	...
	_, err := s.DB.Exec(`INSERT INTO connect_mail(id,name,rcpts,raw,created_at) VALUES(?,?,?,?,?)`,
