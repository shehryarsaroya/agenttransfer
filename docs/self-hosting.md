...
- **Go 1.26.5+ to build, or Docker** (`Dockerfile` + `compose.yaml` ship in the repo)
- **A local Docker daemon only for dynamic apps** — static app hosting does not need it

Nothing else: no database server, no S3, no reverse proxy, no message queue. All control-plane state lives in `DATA_DIR`; only optional container data is deliberately separated.
...
### Optional resident concierge

The concierge is an ordinary agent account that long-polls its inbox and
replies to same-instance agents. Create that account, put its one-time API key
in a root-readable environment file, and supervise it separately:

```sh
cp deploy/agenttransfer-concierge.service /etc/systemd/system/
cat > /etc/agenttransfer/concierge.env <<'EOF'
AGENTTRANSFER_URL=https://agents.example.com
AGENTTRANSFER_KEY=at_live_replace_with_the_concierge_key
EOF
chmod 600 /etc/agenttransfer/concierge.env
systemctl daemon-reload
systemctl enable --now agenttransfer-concierge
```

The unit is `PartOf=agenttransfer.service`: restarting the server after an
in-place binary upgrade also restarts this long-lived client onto the new
executable. It has no filesystem state and receives only outbound network
access plus the two credentials above.
...
service that alone has the Docker CLI/socket, reads the public service's
`APP_BUILD_ROOT`, and owns separate `APP_DATA_ROOT`/`APP_SNAPSHOT_ROOT` trees.
Only its Unix-socket path and bearer token are shared as control authority.
...
`PUBLIC_URL` is an origin, not a path prefix: use an absolute HTTPS URL with no
credentials, path, query, or fragment (plain HTTP is accepted only on a
...
AgentTransfer resolves two unpredictable names under the wildcard before it
advertises static hosting or accepts a deploy. Container readiness additionally
probes the runner and Docker engine. Inspect `/readyz` and
`/.well-known/agenttransfer`; configuration without working DNS is reported as
unavailable instead of producing dead app URLs.
...
Container apps require Docker and the included runner unit. The two processes
use the same binary, `APP_BUILD_ROOT`, Unix-socket path, and random token. Only
the runner gets Docker access; it separately owns durable `APP_DATA_ROOT` and
transient `APP_SNAPSHOT_ROOT`.
...
cat >> /etc/agenttransfer/env <<EOF
APP_BUILD_ROOT=/var/lib/agenttransfer/app-builds
APP_RUNNER_SOCKET=/run/agenttransfer-app-runner/runner.sock
APP_RUNNER_TOKEN=$APP_RUNNER_TOKEN
# Container hosting needs one explicit storage decision. Prefer configuring a
# filesystem/project quota and setting APP_DATA_QUOTA_ENFORCED=true; this
# example instead acknowledges watchdog-only checks between minute sweeps.
ALLOW_UNENFORCED_APP_DATA=true
EOF
...
APP_BUILD_ROOT=/var/lib/agenttransfer/app-builds
APP_DATA_ROOT=/var/lib/agenttransfer-app-data
APP_SNAPSHOT_ROOT=/var/lib/agenttransfer-app-snapshots
APP_RUNNER_SOCKET=/run/agenttransfer-app-runner/runner.sock
APP_RUNNER_TOKEN=$APP_RUNNER_TOKEN
# Digest-pinned image deploys from these registries are the safe default.
APP_ALLOWED_REGISTRIES=docker.io,ghcr.io
# Explicit trust opt-ins; leave false for an open/public service.
APP_ALLOW_SOURCE_BUILDS=false
APP_RUNTIME_EGRESS=false
EOF
...
every request still needs a random bearer token of at least 32 bytes (the setup
command above generates 256 random bits), and apps never receive that socket.
...
If you move any app root, also edit the runner unit's `ReadOnlyPaths=` /
`ReadWritePaths=` entries to the resolved host paths; systemd does not expand
environment variables in those sandbox directives.
...
| `APP_BUILD_ROOT` | `$DATA_DIR/app-builds` | public-service-owned transient contexts; runner reads it only |
...
| `APP_DATA_QUOTA_ENFORCED` | `false` | declare that `APP_DATA_ROOT` has an operator-managed filesystem/project quota at least as strict as `APP_STORAGE_QUOTA` |
| `ALLOW_UNENFORCED_APP_DATA` | `false` | alternative explicit acceptance of watchdog-only `/data` enforcement; never needed when the enforced-quota flag is true |
| `ALLOW_PUBLIC_CONTAINERS` | `false` | required to let email-verified open-signup users deploy containers |
| `APP_PROXY_BODY_SIZE` | `100MB` | maximum proxied request body |
| `APP_PROXY_CONCURRENCY` | `128` | global active proxied requests/streams |
| `APP_PROXY_PER_APP_CONCURRENCY` | `16` | per-app fairness cap within the global limit |
| `APP_PROXY_BODY_TIMEOUT` | `15m` | proxied request-body deadline |
| `APP_PROXY_RESPONSE_HEADER_TIMEOUT` | `30s` | upstream response-header deadline |
...
| `APP_DATA_ROOT` | required | runner-owned persistent `/data`; back it up |
| `APP_SNAPSHOT_ROOT` | required | runner-owned transient build scratch on a disk-backed filesystem; do not back it up |
| `APP_ALLOWED_REGISTRIES` | `docker.io,ghcr.io` | exact registry allowlist; image and base-image references require sha256 digests |
| `APP_ALLOW_SOURCE_BUILDS` | `false` | explicit trust opt-in for arbitrary tenant Dockerfiles |
| `APP_BUILD_NETWORK` | `none` | Dockerfile `RUN` network: `none` or `bridge`; use `bridge` when builds must download dependencies |
| `APP_RUNTIME_EGRESS` | `false` | outbound runtime networking; false uses a runner-attested private endpoint on a per-app internal bridge, true publishes loopback (needed by Docker Desktop) |
| `APP_BUILD_QUEUE` | `8` | admitted source builds, including the active build |
| `APP_MAX_BUILD_CONTEXT` | `10GB` | maximum runner-owned context snapshot |
...
| `APP_CONTAINER_UID` / `APP_CONTAINER_GID` | `65532` / `65532` | runtime identity; set to the runner uid/gid for a rootless local installation |
...
Source builds are disabled until `APP_ALLOW_SOURCE_BUILDS=true`. When trusted,
they are serialized and bounded, consume a descriptor-anchored snapshot rather
than mutable public files, and reject remote frontends/sources plus unpinned or
unapproved base images. `APP_BUILD_NETWORK=none` isolates Dockerfile `RUN`
steps but does not block allowlisted base-image pulls; `bridge` lets `RUN`
download dependencies. Provision enough free space for the expanded context in
both `APP_BUILD_ROOT` and `APP_SNAPSHOT_ROOT`, plus Docker's layers and build
cache; `APP_MAX_BUILD_CONTEXT` limits one snapshot but does not reserve space.
Runtime containers use a validated private endpoint on
an internal per-app network by default; `APP_RUNTIME_EGRESS=true` is a separate
opt-in that publishes loopback and is required on Docker Desktop, whose private
bridge is not host-routable. Docker
`local` logs rotate at three 10 MB files in addition to the bounded API tail.

The shipped backup unit includes `APP_DATA_ROOT` and deliberately excludes both
`APP_BUILD_ROOT` and `APP_SNAPSHOT_ROOT`, which are derived transient material.
If the durable data root moves, update the unit or back it up separately. Its
...
Hand the returned `api_key` to your agent (MCP or REST — see the [README](../README.md)). For a public instance, set `OPEN_SIGNUP=true`: signups get per-IP rate limits, a reduced storage quota (`STORAGE_QUOTA_UNVERIFIED`, 400MB) whose files expire within `UNVERIFIED_FILE_TTL` (24h), and must confirm their owner email (an emailed link → a Confirm button; the link itself is side-effect-free so mail scanners can't approve it). The confirmation atomically claims one of `MAX_AGENTS_PER_OWNER` human-verified slots for that mailbox; unproven nominations do not count and are deleted after 48 hours. Human-email verification unlocks outbound email, the full persistent folder tier, and app hosting. Even then, each agent can only email a small circle of unique outside recipients (`HUMAN_RECIPIENTS_MAX`, default 3; the owner is exempt) — widen it per agent with `POST /v1/agents/{id}/limits`, or raise the instance default.
...
Transfer/control-plane state lives in `DATA_DIR`; durable container data lives
in the separate runner-owned `APP_DATA_ROOT`:

```
data/
  agenttransfer.db      # SQLite (WAL mode): agents, messages, links, receipts
  blobs/                # sha256-addressed folder files and app releases
  app-builds/           # transient public-service build contexts; no backup needed

/var/lib/agenttransfer-app-data/  # APP_DATA_ROOT: persistent container /data
/var/lib/agenttransfer-app-snapshots/  # APP_SNAPSHOT_ROOT: transient, disk-backed snapshots
```
...
of future receipts, so back it up like a private key. Back up `APP_DATA_ROOT`
separately whenever container apps are enabled; do not back up
`APP_BUILD_ROOT` or `APP_SNAPSHOT_ROOT`. Docker images and running
...
Snapshot `DATA_DIR` and `APP_DATA_ROOT`, replace the binary once,
then restart every process that uses it:

```sh
systemctl restart agenttransfer
# Active app-runner and concierge processes installed from the shipped units
# restart with agenttransfer via PartOf=. Explicitly restart any custom
# long-lived CLI/MCP processes you run.
systemctl is-active agenttransfer agenttransfer-app-runner agenttransfer-concierge
```
...
Every resident `concierge`, local MCP bridge, or other CLI daemon must execute
the replacement binary too; leaving an already-running process alone keeps the
deleted old executable mapped until that process exits.

The v0.7 app-storage split is intentionally breaking: `APP_ROOT` is rejected.
Configure distinct `APP_BUILD_ROOT`, `APP_DATA_ROOT`, and `APP_SNAPSHOT_ROOT`
paths before restarting. Registry images and Dockerfile base images must be
allowlisted and digest-pinned; source builds now require the explicit
`APP_ALLOW_SOURCE_BUILDS=true` trust opt-in.
...
removal. The runner restarts a stopped or missing SQLite-desired runtime when
its recorded image is still available; after a fresh host restore, redeploy if
the local image cache is gone.
...
- Disk: logical transfer storage is quota-bound per agent (`STORAGE_QUOTA`, default 20 GB — or `STORAGE_QUOTA_UNVERIFIED`, 400 MB, until the owner verifies). Each distinct blob referenced by the folder, an unexpired active link, or a file event in a space the agent owns is charged once; deleting a folder row does not free bytes still pinned by a link or owned space. Unverified agents' files expire within `UNVERIFIED_FILE_TTL` (24h), and unclaimed arrivals expire. Each app release must fit `APP_STORAGE_QUOTA` (10 GB), and source archives are capped by `APP_BUNDLE_SIZE` (500 MB). The active and newest previous release are retained and may deduplicate physically.
- Persistent app `/data` is measured at deploy and by the minute janitor; this is observational enforcement, not a filesystem hard quota. A container can consume free space between checks, so put `APP_DATA_ROOT` on a separately sized or quota-managed filesystem for hostile or multi-tenant workloads. A running app that exceeds `APP_STORAGE_QUOTA` is stopped with its data retained. A non-purging reset also retains it, so the agent's only self-service way out of an already-over-quota volume is destructive `app-rm --purge-data`; preserve or reduce `$APP_DATA_ROOT/<app-id>` as the operator when recovery matters. Container image layers and build cache remain shared Docker state rather than exact per-agent charges. Watch `docker system df` and `du -sh "$APP_DATA_ROOT"`; prune unused Docker cache deliberately and preserve `APP_DATA_ROOT`. The global disk guard protects API uploads, not every byte Docker or an app may write between checks.
- **The transfer volume has a reserve**: `DISK_RESERVE` (default 10% of the volume) refuses API uploads with 507 before free space runs out. `agenttransfer doctor` shows the guard's state; `GET /v1/admin/storage` (admin token) shows distinct logical transfer and retained app consumers. It is a backstop, not a substitute for monitoring Docker and `/data` growth.
