...
// extraDenyPrefixes are ranges Go's netip helpers don't classify but that must
// still be blocked: CGNAT, benchmarking, NAT64, plus the IPv6 transition ranges
// that can embed an IPv4 internal target (v4-compatible, 6to4, Teredo).
var extraDenyPrefixes = []netip.Prefix{
	netip.MustParsePrefix("100.64.0.0/10"), // CGNAT
	netip.MustParsePrefix("198.18.0.0/15"), // benchmarking
	netip.MustParsePrefix("64:ff9b::/96"),  // NAT64
	netip.MustParsePrefix("::/96"),         // IPv4-compatible IPv6 (deprecated)
	netip.MustParsePrefix("2002::/16"),     // 6to4 (can wrap a private v4)
...
func publicUnicast(ip netip.Addr) bool {
	...
	if !ip.IsValid() || ip.IsLoopback() || ip.IsPrivate() || // IsPrivate = RFC1918 + fc00::/7
		ip.IsLinkLocalUnicast() || ip.IsLinkLocalMulticast() || // 169.254/16 (incl. cloud metadata), fe80::/10
		ip.IsMulticast() || ip.IsUnspecified() || ip.IsInterfaceLocalMulticast() {
		return false
	}
	for _, p := range extraDenyPrefixes {
		if p.Contains(ip) {
			return false
		}
	}
...
func newWebhookSecret() string { return "whsec_" + randB64(24) }
...
func (s *Server) deliverOne(client *http.Client, d store.WebhookDelivery) {
	...
	req.Header.Set("Webhook-Signature", signWebhook(d.Secret, d.ID, ts, d.Payload))
